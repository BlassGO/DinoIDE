;By HotKeyIt -> https://www.autohotkey.com/boards/viewtopic.php?f=6&t=3573
;I just add a 9-byte header to identify the saved DinoIDE objects (BlassGO)

ObjDump(obj,ByRef var:="",mode:=0,header:="_"){
  If IsObject(var){ ; FileAppend mode
    FileDelete, % obj
    f:=FileOpen(obj,"rw-rwd","CP0"),f.Write("DinoIDE_" . header),VarSetCapacity(v,sz:=RawObjectSize(var,mode)+8,0)
...
ObjHeader(file){
    if !FileExist(file)
       return
    f:=FileOpen(file, "r")
    return (InStr(header:=f.Read(9),"DinoIDE_")?header:""),f.Close()
}
