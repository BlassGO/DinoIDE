...
#include %A_ScriptDir%\src\core\console.ahk
...
class DinoIDE
{
	...
	UpdateStatusBar()
	{
		...
		Len := this.RichCode.SendMsg(0x45F, &GTL, 0) ; EM_GETTEXTLENGTHEX
		...
		if (this.Settings.Gutter.Width)
			this.UpdateGutterIfNeeded()
	}

	UpdateGutterIfNeeded()
	{
	...
	GuiSize()
	{
		...
		if (this.Console && this.Console.IsActive()) {
	...
	UpdateConsolePosition() {
		...
		DllCall("GetClientRect", "UPtr", this.hMainWindow, "Ptr", &RECT, "UInt")
		mainWidth := NumGet(RECT, 8, "Int")
		mainHeight := NumGet(RECT, 12, "Int")
		
		consoleWidth := mainWidth // 2
		consoleHeight := mainHeight - this.StatusBarHeight - 60
		consoleX := mainWidth - consoleWidth
		consoleY := 0 
		
		DllCall("SetWindowPos", "Ptr", this.Console.hConsoleWindow, "Ptr", 0
			, "Int", consoleX, "Int", consoleY, "Int", consoleWidth, "Int", consoleHeight
			, "UInt", 0x40)  ; SWP_SHOWWINDOW|SWP_NOACTIVATE|SWP_NOSENDCHANGING
	}	
