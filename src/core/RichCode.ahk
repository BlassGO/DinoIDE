...
class RichCode
{
   ...
   FileSaveAs(isnew:=false) {
		If (File := this.FileDlg("S")) {
			if isnew {
				if (this.Open_File && !this.FileClose())
					return 0
				this.Value:=":main args`n`t"
				this.Selection:=[12,12]
			}
   ...
   Add_Recent(File:="",GetPos:="") {
		...
		if File {
			...
			IniWrite, %new_list%, %CONFIG_PATH%, GENERAL, last_opened
			Menu, Opened, Add, %File%, Open_Recent
			Gui, +LastFound
			WinGetTitle, Title
			StringSplit, Title, Title, -, %A_Space%
			WinSetTitle, %Title1% - %File%
			this.Open_File:=File, Arg_in(File)
		}
		IniRead, last_opened_list, %CONFIG_PATH%, GENERAL, last_opened_list, 0
		...
		IniWrite, %new_list%, %CONFIG_PATH%, GENERAL, last_opened_list
	}
   ...
   FileDlg(Mode, File := "") { ; Open and save as dialog box
      ...
      ; File : optional file name
		...
		Static FilterN1 := "DinoScript (.dino)",   FilterP1 :=  "*.dino", DefExt := "dino", DefFilter := 1
